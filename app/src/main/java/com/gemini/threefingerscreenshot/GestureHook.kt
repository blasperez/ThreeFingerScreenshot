...
class GestureHook : IXposedHookLoadPackage {
    ...
    private var lastMoveEventTime: Long = -1L
    private var lastTriggeredEventTime: Long = -1L
    ...
    // --- Tuning ---
    private val swipeThreshold = 200     // Pixels to swipe down
    private val maxGestureTime = 1000L   // Max ms to complete swipe
    private val screenshotCooldown = 1800L // ms between screenshots
        ...
        synchronized(gestureLock) {
            when (event.actionMasked) {
                ...
                MotionEvent.ACTION_MOVE -> {
                    if (isSwiping && event.pointerCount == 3) {
                        // The same MotionEvent can arrive through multiple hooks.
                        if (event.eventTime == lastMoveEventTime) return
                        lastMoveEventTime = event.eventTime

                        val currentY = averageY(event)
                        val deltaY = currentY - startY
                        val elapsed = SystemClock.elapsedRealtime() - gestureStartTime
                        val now = SystemClock.elapsedRealtime()

                        if (deltaY > swipeThreshold
                            && elapsed < maxGestureTime
                            && (now - lastScreenshotTime) > screenshotCooldown
                            && event.eventTime != lastTriggeredEventTime
                        ) {
                            log("Three-finger swipe DOWN detected! Taking screenshot.")
    ...
    companion object {
        private const val TAG = "ThreeFingerScreenshot"
